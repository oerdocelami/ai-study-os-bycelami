...
export default function LibraryPage() {
  ...
  const generatePDFCards = async (pdf: any) => {
    ...
    try {
      // Split extracted text into chunks and create flashcards
      ...
      // Create additional flashcards from text
  ...
  const deletePDF = async (id: string, fileUrl: string) => {
    ...
    try {
      ...
      if (filePath) {
        await supabase.storage.from("pdfs").remove([`pdfs/${filePath}`]);
      }
          ...
          <div className="max-w-6xl mx-auto">
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <a
                  href="/upload"
                  className="inline-block bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-600 hover:to-cyan-600 text-white font-semibold px-8 py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl"
                >
                  Upload First PDF
                </a>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {pdfs.map((pdf) => (
                    <div
                      key={pdf.id}
                      className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl border border-slate-700/50 p-6 hover:border-emerald-500/50 transition-all duration-300 transform hover:scale-105 hover:shadow-xl"
                    >
                      ...
                      <div className="relative z-10">
                        ...
                        {/* File Name */}
                        <h3 className="text-lg font-semibold text-white mb-4 line-clamp-2 group-hover:text-emerald-300 transition-colors">
                          {pdf.name}
                        </h3>
                        ...
                        <div className="space-y-3">
                          <div className="flex gap-2">
                            <a
                              ...
                              className="flex-1 flex items-center justify-center gap-2 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 font-medium py-2 rounded-lg transition-all"
                            >
                            ...
                            <a
                              href={pdf.file_url}
                              download
                              className="flex-1 flex items-center justify-center gap-2 bg-slate-700/30 hover:bg-slate-600/50 text-slate-300 font-medium py-2 rounded-lg transition-all"
                            >
                          ...
                          <button
                            ...
                            className="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-medium py-2 rounded-lg transition-all"
                          >
                            Generate Flashcards
                          </button>
                        </div>
